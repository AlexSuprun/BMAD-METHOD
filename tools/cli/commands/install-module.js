...
module.exports = {
  ...
  action: async (modulePath, options) => {
    ...
    try {
      ...
      // Step 3: Collect module configuration from user
      const configCollector = new ConfigCollector();
      configCollector.customModulePaths = new Map([[moduleCode, absolutePath]]);

      const projectDir = path.dirname(bmadDir);
      await configCollector.loadExistingConfig(projectDir);

      // Use existing core config (BMAD is already installed, so core is configured)
      const collectedCoreConfig = configCollector.existingConfig.core || {};

      // Collect configuration for this module only (shows prompts if module.yaml has them)
      await configCollector.collectModuleConfig(moduleCode, projectDir);

      const collectedModuleConfig = configCollector.collectedConfig[moduleCode] || {};
      ...
      // Step 5: Install module using ModuleManager
      ...
      await moduleManager.install(moduleCode, bmadDir, null, {
        ...
        coreConfig: collectedCoreConfig,
        installedIDEs: manifestData.ides || [],
      });

      spinner.succeed('Module files installed');

      // Write module config.yaml
      const moduleTargetDir = path.join(bmadDir, moduleCode);
      const configYamlPath = path.join(moduleTargetDir, 'config.yaml');

      // Merge core config into module config (like normal installer does)
      const finalConfig = { ...collectedModuleConfig, ...collectedCoreConfig };

      // Only write config if there are any values
      if (Object.keys(finalConfig).length > 0) {
        const packageJson = require(path.join(getProjectRoot(), 'package.json'));
        const configHeader = `# ${moduleCode.toUpperCase()} Module Configuration
# Generated by BMAD installer
# Version: ${packageJson.version}
# Date: ${new Date().toISOString()}

`;
        const configContent = configHeader + yaml.stringify(finalConfig, { indent: 2, lineWidth: 0 });
        await fs.writeFile(configYamlPath, configContent.endsWith('\n') ? configContent : configContent + '\n');
      }
      ...
      // Step 7: Regenerate IDE commands
